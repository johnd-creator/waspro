...
<div class="p-4 sm:p-6 lg:p-8">
    <div class="mb-6 rounded-2xl border shadow-sm"
        ...
        <div class="flex items-center justify-between border-b px-6 py-5" style="border-color: var(--border-primary);">
            <h3 class="text-xl font-semibold" style="color: var(--text-primary);"><i
                    class="fas fa-exclamation-triangle mr-2"></i>Laporan Kadaluarsa</h3>
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
                <a href="{{ route('expiry-reports.index', ['expiry_status' => 'Expired']) }}"
                    class="rounded-lg p-4 transition-transform hover:scale-105 cursor-pointer"
                    style="background-color: var(--danger-bg);">
                    <div class="flex items-center">
                        <div class="mr-4 text-2xl" style="color: var(--danger-primary);"><i
                        ...
                        <div>
                            ...
                            <div class="text-2xl font-bold" style="color: var(--text-primary);">
                                {{ $summary['expired'] }}</div>
                            <div class="text-xs mt-1" style="color: var(--danger-primary);">Limbah sudah melewati batas
                            </div>
                        </div>
                    </div>
                </a>
                <a href="{{ route('expiry-reports.index', ['expiry_status' => 'Critical']) }}"
                    class="rounded-lg p-4 transition-transform hover:scale-105 cursor-pointer"
                    style="background-color: var(--critical-bg);">
                    <div class="flex items-center">
                        <div class="mr-4 text-2xl" style="color: var(--critical-primary);"><i
                                class="fas fa-exclamation-triangle"></i></div>
                        <div>
                            <div class="text-sm font-medium" style="color: var(--critical-primary);">Kritis</div>
                            <div class="text-2xl font-bold" style="color: var(--text-primary);">
                                {{ $summary['critical'] }}</div>
                            <div class="text-xs mt-1" style="color: var(--critical-primary);">Tersisa < 7 hari (H-1 s/d
                                    H-7)</div>
                            </div>
                        </div>
                </a>
                <a href="{{ route('expiry-reports.index', ['expiry_status' => 'Warning']) }}"
                    class="rounded-lg p-4 transition-transform hover:scale-105 cursor-pointer"
                    style="background-color: var(--warning-bg);">
                    <div class="flex items-center">
                        <div class="mr-4 text-2xl" style="color: var(--warning-primary);"><i class="fas fa-clock"></i>
                        </div>
                        <div>
                ...
                <a href="{{ route('expiry-reports.index', ['expiry_status' => 'Safe']) }}"
                    class="rounded-lg p-4 transition-transform hover:scale-105 cursor-pointer"
                    style="background-color: var(--success-bg);">
                    <div class="flex items-center">
                        <div class="mr-4 text-2xl" style="color: var(--success-primary);"><i
                                class="fas fa-check-circle"></i></div>
                        <div>
                            <div class="text-sm font-medium" style="color: var(--success-primary);">Aman</div>
                            <div class="text-2xl font-bold" style="color: var(--text-primary);">{{ $summary['safe'] }}
                            </div>
            ...
            <form method="GET" action="{{ route('expiry-reports.index') }}" id="filterForm" class="mb-6">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
                    <div>
                        <label for="expiry_status" class="mb-1 block text-sm font-medium"
                            style="color: var(--text-secondary);">Status Kadaluarsa</label>
                        <select name="expiry_status" id="expiry_status"
                            ...
                            <option value="Expired" {{ request('expiry_status') == 'Expired' ? 'selected' : '' }}>
                                Kadaluarsa</option>
                            <option value="Critical" {{ request('expiry_status') == 'Critical' ? 'selected' : '' }}>Kritis
                                (< 7 hari)</option>
                            <option value="Warning" {{ request('expiry_status') == 'Warning' ? 'selected' : '' }}>
                                Peringatan (8-30 hari)</option>
                            <option value="Safe" {{ request('expiry_status') == 'Safe' ? 'selected' : '' }}>Aman (> 30
                                hari)</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        <label for="perusahaan_id" class="mb-1 block text-sm font-medium"
                            style="color: var(--text-secondary);">Perusahaan</label>
                    ...
                    <div>
                        <label for="date_from" class="mb-1 block text-sm font-medium"
                            style="color: var(--text-secondary);">Tanggal Masuk Dari</label>
                    ...
                    <div>
                        <label for="date_to" class="mb-1 block text-sm font-medium"
                            style="color: var(--text-secondary);">Tanggal Masuk Sampai</label>
                ...
                <div class="mt-4 flex items-center gap-2">
                    <button type="submit"
                        class="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-white shadow-sm hover:bg-blue-700">
                        <i class="fas fa-search"></i> Filter
                    </button>
                    <a href="{{ route('expiry-reports.index') }}"
                        class="inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-2 shadow-sm"
            ...
            <table class="min-w-full w-full">
                <thead style="background-color: var(--border-primary);">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                            style="color: var(--text-secondary);">Tgl Masuk</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                            style="color: var(--text-secondary);">Kode</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                            style="color: var(--text-secondary);">Tanggal Kadaluarsa</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                            style="color: var(--text-secondary);">Sisa Waktu</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                            style="color: var(--text-secondary);">Status Limbah</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                ...
                <tbody>
                    ...
                    <tr class="border-b" style="border-color: var(--border-primary);"
                        ...
                        <td class="px-4 py-3 text-sm font-medium" style="color: var(--text-primary);">
                            {{ $log->kode_limbah }}</td>
                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                            {{ $log->jenisLimbah->nama_limbah ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-sm font-medium" style="color: var(--text-primary);">
                            {{ number_format($log->jumlah_limbah_masuk, 2) }}</td>
                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                            {{ $log->perusahaanPenghasil->nama_perusahaan ?? 'N/A' }}</td>
                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                        ...
                        <td class="px-4 py-3 text-sm">
                            ...
                            @if($daysLeft !== null)
                                ...
                                @elseif($daysLeft <= 7)
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium"
                                        style="background-color: var(--critical-bg); color: var(--critical-primary);">H-{{ $daysLeft }}</span>
                                @elseif($daysLeft <= 30)
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium"
                                        style="background-color: var(--warning-bg); color: var(--warning-primary);">H-{{ $daysLeft }}</span>
                                @else
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium"
                                        style="background-color: var(--success-bg); color: var(--success-primary);">H-{{ $daysLeft }}</span>
                                @endif
                            @else
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium"
                        ...
                        <td class="px-4 py-3 text-sm">
                            <a href="{{ route('log-penyimpanan.show', $log) }}"
                                class="inline-flex items-center justify-center rounded-lg p-2 hover:bg-gray-200 dark:hover:bg-gray-700">
