...
@section('content')
    <div class="p-4 sm:p-6 lg:p-8">
        <!-- Header Section -->
        <div class="rounded-2xl shadow-sm border mb-6"
            ...
            <div class="px-6 py-6 flex justify-between items-center" style="border-color: var(--border-primary);">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('application-settings.edit', $setting) }}"
                        class="inline-flex items-center px-6 py-3 text-white font-medium rounded-xl transition-all duration-200 shadow-lg"
                    ...
                    <a href="{{ route('application-settings.index') }}"
                        class="inline-flex items-center px-6 py-3 font-medium rounded-xl transition-all duration-200 shadow-lg"
                        ...
                        @elseif($setting->type === 'json')
                            <div class="position-relative">
                                ...
                                <pre class="bg-light p-3 rounded"
                                    id="json-value"><code>{{ json_encode(json_decode($setting->value), JSON_PRETTY_PRINT) }}</code></pre>
                            </div>
                        @else
                            <div class="position-relative">
                                <button class="btn btn-sm btn-outline-secondary position-absolute top-0 end-0 m-2"
    ...
    $value = ApplicationSetting::getValue('{{ $setting->key }}');

    // Atau menggunakan config helper (jika sudah di-cache)
    config('{{ $setting->key }}');

    // Set nilai baru
    ApplicationSetting::setValue('{{ $setting->key }}', $newValue);</code></pre>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            @foreach($relatedSettings as $related)
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <div>
                                        <a href="{{ route('application-settings.show', $related) }}" class="text-decoration-none">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="text-danger"><small>Tindakan ini tidak dapat dibatalkan dan dapat mempengaruhi fungsi
                                aplikasi.</small></p>
                    </div>
