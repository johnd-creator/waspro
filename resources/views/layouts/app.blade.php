...
<body style="background-color: var(--bg-primary);">
    <div class="flex h-screen">
        @auth
            <!-- Sidebar -->
            <div class="fixed inset-y-0 left-0 z-50 w-64 transform -translate-x-full md:translate-x-0 transition-all duration-300 ease-in-out shadow-2xl"
                id="sidebar"
                style="background: linear-gradient(to bottom right, var(--sidebar-bg-start), var(--sidebar-bg-via), var(--sidebar-bg-end));">
                <nav class="h-full flex flex-col">
                    <div
                        class="flex items-center justify-center h-16 px-4 border-b border-slate-700/50 bg-slate-800/50 backdrop-blur-sm">
                    ...
                    <div
                        class="flex-1 px-2 py-3 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent">
                        <div class="space-y-3">
                            <!-- Menu Dashboard -->
                            <div class="space-y-1">
                                <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('dashboard') ? 'bg-blue-600/80 text-white shadow-lg shadow-blue-600/25' : '' }}"
                                    href="{{ route('dashboard') }}">
                                    <div
                                        class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-blue-500/50 transition-colors duration-200">
                            ...
                            <div class="space-y-1">
                                <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('log-penyimpanan.*') ? 'bg-green-600/80 text-white shadow-lg shadow-green-600/25' : '' }}"
                                    href="{{ route('log-penyimpanan.index') }}">
                                    <div
                                        class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-green-500/50 transition-colors duration-200">
                            ...
                            @if(Auth::guard('web')->user() && (Auth::guard('web')->user()->isSupervisor() || Auth::guard('web')->user()->isAdmin()))
                                ...
                                <div class="space-y-1">
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('pengangkutan-limbah.*') ? 'bg-yellow-600/80 text-white shadow-lg shadow-yellow-600/25' : '' }}"
                                        href="{{ route('pengangkutan-limbah.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-yellow-500/50 transition-colors duration-200">
                            ...
                            <!-- Section LAPORAN -->
                            <div class="section-header cursor-pointer px-3 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/30 mb-2 mt-4 flex items-center justify-between hover:text-slate-300 transition-colors"
                                onclick="toggleSection('laporan')"
                                data-section="laporan">
                                <div>
                                    <i class="fas fa-chart-bar mr-1"></i>LAPORAN
                                </div>
                                <i class="fas fa-chevron-down text-xs transition-transform duration-200" id="chevron-laporan"></i>
                            </div>
                            <div id="section-laporan" class="section-content overflow-hidden transition-all duration-300" style="max-height: 0;">
                                <div class="space-y-1 pl-2">
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('reports.*') ? 'bg-purple-600/80 text-white shadow-lg shadow-purple-600/25' : '' }}"
                                        href="{{ route('reports.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-purple-500/50 transition-colors duration-200 relative">
                                            <i class="fas fa-file-alt text-xs"></i>
                                            <span
                                                class="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full text-xs flex items-center justify-center"
                                                id="sidebar-notification-badge" style="display: none;"></span>
                                        </div>
                            ...
                            <div class="section-header cursor-pointer px-3 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/30 mb-2 mt-4 flex items-center justify-between hover:text-slate-300 transition-colors"
                                ...
                                <i class="fas fa-chevron-down text-xs transition-transform duration-200" id="chevron-master-data"></i>
                            </div>
                            <div id="section-master-data" class="section-content overflow-hidden transition-all duration-300" style="max-height: 0;">
                                <div class="space-y-1 pl-2">
                                    <!-- Menu Perusahaan Penghasil -->
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('perusahaan-penghasil.*') ? 'bg-purple-600/80 text-white shadow-lg shadow-purple-600/25' : '' }}"
                                        href="{{ route('perusahaan-penghasil.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-purple-500/50 transition-colors duration-200">
                                            <i class="fas fa-building text-xs"></i>
                                        </div>
                                    ...
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('unit-pembangkit.*') ? 'bg-purple-600/80 text-white shadow-lg shadow-purple-600/25' : '' }}"
                                        href="{{ route('unit-pembangkit.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-purple-500/50 transition-colors duration-200">
                                    ...
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('pengguna-sistem.*') ? 'bg-purple-600/80 text-white shadow-lg shadow-purple-600/25' : '' }}"
                                        href="{{ route('pengguna-sistem.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-purple-500/50 transition-colors duration-200">
                                    ...
                                    <!-- Menu Peran Pengguna -->
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('peran-pengguna.*') ? 'bg-purple-600/80 text-white shadow-lg shadow-purple-600/25' : '' }}"
                                        href="{{ route('peran-pengguna.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-purple-500/50 transition-colors duration-200">
                            ...
                            <!-- Section LIMBAH -->
                            <div class="section-header cursor-pointer px-3 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/30 mb-2 mt-4 flex items-center justify-between hover:text-slate-300 transition-colors"
                                onclick="toggleSection('limbah')"
                                data-section="limbah">
                                <div>
                                    <i class="fas fa-recycle mr-1"></i>LIMBAH
                                </div>
                                <i class="fas fa-chevron-down text-xs transition-transform duration-200" id="chevron-limbah"></i>
                            </div>
                            <div id="section-limbah" class="section-content overflow-hidden transition-all duration-300" style="max-height: 0;">
                                <div class="space-y-1 pl-2">
                                    <!-- Menu Jenis Limbah -->
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('jenis-limbah.*') ? 'bg-orange-600/80 text-white shadow-lg shadow-orange-600/25' : '' }}"
                                    ...
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('karakteristik-limbah.*') ? 'bg-orange-600/80 text-white shadow-lg shadow-orange-600/25' : '' }}"
                                        href="{{ route('karakteristik-limbah.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-orange-500/50 transition-colors duration-200">
                                    ...
                                    <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('kategori-kegiatan-sumber.*') ? 'bg-orange-600/80 text-white shadow-lg shadow-orange-600/25' : '' }}"
                                        href="{{ route('kategori-kegiatan-sumber.index') }}">
                                        <div
                                            class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-orange-500/50 transition-colors duration-200">
                            ...
                            <div class="section-header cursor-pointer px-3 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/30 mb-2 mt-4 flex items-center justify-between hover:text-slate-300 transition-colors"
                                ...
                                <i class="fas fa-chevron-down text-xs transition-transform duration-200" id="chevron-pengaturan"></i>
                            </div>
                            <div id="section-pengaturan" class="section-content overflow-hidden transition-all duration-300" style="max-height: 0;">
                                <div class="space-y-1 pl-2">
                                    @if(Auth::guard('web')->user() && Auth::guard('web')->user()->isSuperAdmin())
                                        <!-- Menu System Settings -->
                                        <a class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-slate-700/50 hover:text-white hover:shadow-lg hover:scale-105 {{ request()->routeIs('application-settings.*') ? 'bg-blue-600/80 text-white shadow-lg shadow-blue-600/25' : '' }}"
                                            href="{{ route('application-settings.index') }}">
                                            <div
                                                class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-blue-500/50 transition-colors duration-200">
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit"
                                            class="nav-link group flex items-center px-3 py-2 text-slate-200 rounded-lg transition-all duration-200 hover:bg-red-600/50 hover:text-white hover:shadow-lg hover:scale-105 w-full text-left">
                                            <div
                                                class="w-6 h-6 bg-slate-700/50 rounded-md flex items-center justify-center mr-2 group-hover:bg-red-500/50 transition-colors duration-200">
            ...
            <div class="flex-1 flex flex-col overflow-hidden md:ml-64 transition-all duration-300 ease-in-out">
                <!-- Top Navbar -->
                <nav class="backdrop-blur-sm border-b shadow-sm sticky top-0 z-40"
                    style="background-color: var(--navbar-bg); border-color: var(--border-primary);">
                    <div class="flex items-center justify-between px-6 py-4">
                        <!-- Left side -->
                        <div class="flex items-center space-x-4">
                            <button class="md:hidden focus:outline-none p-2 rounded-lg transition-colors duration-200"
                                id="sidebarToggle" style="color: var(--text-secondary);"
                                onmouseover="this.style.color='var(--text-primary)'; this.style.backgroundColor='var(--hover-bg)'"
                        ...
                        <div class="flex items-center space-x-4"> <!-- Search -->
                            ...
                            <div class="relative">
                                <button
                                    class="flex items-center justify-center w-10 h-10 focus:outline-none rounded-xl transition-all duration-200"
                                    id="darkModeToggle" onclick="toggleDarkMode()" style="color: var(--text-secondary);"
                                    onmouseover="this.style.color='var(--text-primary)'; this.style.backgroundColor='var(--hover-bg)'"
                                    onmouseout="this.style.color='var(--text-secondary)'; this.style.backgroundColor='transparent'">
                            ...
                            <div class="relative">
                                <button
                                    class="flex items-center justify-center w-10 h-10 focus:outline-none rounded-xl transition-all duration-200 relative"
                                    id="notification-bell" onclick="toggleNotificationDropdown()"
                                ...
                                <div class="absolute right-0 mt-3 w-96 rounded-2xl shadow-2xl z-50 hidden overflow-hidden notification-dropdown"
                                    id="notification-dropdown"
                                    style="background-color: var(--card-bg); border: 1px solid var(--border-primary);">
                                    <div class="px-6 py-4 border-b"
                                        style="background: linear-gradient(to right, var(--bg-tertiary), var(--bg-tertiary)); border-color: var(--border-primary);">
                                        <div class="flex items-center justify-between">
                                            <h3 class="font-bold" id="notification-header"
                                                style="color: var(--text-primary);">0 Notifikasi</h3>
                                            <button class="text-sm font-medium" onclick="refreshNotifications()"
                                    ...
                                    <div class="border-t"
                                        style="border-color: var(--border-primary); background: var(--card-secondary-bg);">
                                        <a href="{{ route('notifications.index') }}"
                                            class="flex items-center justify-center w-full px-6 py-3 transition-all duration-200 font-medium"
                                            style="color: var(--accent-primary);"
                                            onmouseover="this.style.backgroundColor='var(--hover-bg)'; this.style.color='var(--text-primary)';"
                                            onmouseout="this.style.backgroundColor='var(--card-secondary-bg)'; this.style.color='var(--accent-primary)';">
                            ...
                            <div class="relative"> <button
                                    class="flex items-center space-x-3 focus:outline-none p-2 rounded-xl transition-all duration-200"
                                    id="userDropdown" onclick="toggleUserDropdown()" style="color: var(--text-secondary);"
                                    onmouseover="this.style.color='var(--text-primary)'; this.style.backgroundColor='var(--hover-bg)';"
                                    onmouseout="this.style.color='var(--text-secondary)'; this.style.backgroundColor='transparent';">
                                    <div class="flex items-center space-x-3">
                                        <div
                                            class="w-9 h-9 bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl flex items-center justify-center text-sm font-bold shadow-lg">
                                        ...
                                        <div class="hidden md:block text-left">
                                            <div class="font-semibold text-sm" style="color: var(--text-primary);">
                                                {{ Auth::user()->nama_lengkap ?? Auth::user()->name }}</div>
                                            <div class="text-xs" style="color: var(--text-secondary);">
                                                {{ Auth::user()->unitPembangkit->nama_unit ?? 'N/A' }}</div>
                                        </div>
                                        <i class="fas fa-chevron-down text-xs" style="color: var(--text-secondary);"></i>
                                    </div>
                                </button>
                                <div class="absolute right-0 mt-3 w-80 rounded-2xl shadow-2xl z-50 hidden overflow-hidden"
                                    id="userDropdownMenu"
                                    style="background-color: var(--card-bg); border: 1px solid var(--border-primary);">
                                    <div class="px-6 py-5 border-b"
                                        style="background: linear-gradient(135deg, var(--gradient-start, #dbeafe) 0%, var(--gradient-end, #e0e7ff) 100%); border-color: var(--border-primary);">
                                        <div class="flex items-center space-x-4">
                                            <div
                                                class="w-14 h-14 bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-2xl flex items-center justify-center text-xl font-bold shadow-lg">
                                                {{ strtoupper(substr(Auth::user()->nama_lengkap ?? Auth::user()->name, 0, 1)) }}
                                            </div>
                                            <div class="flex-1">
                                                ...
                                                <div class="text-sm" style="color: var(--text-secondary);">
                                                    {{ Auth::user()->email_address ?? Auth::user()->email }}</div>
                                    ...
                                    <div class="py-3">
                                        <a class="flex items-center px-6 py-3 transition-all duration-200 group"
                                            href="{{ route('profile.show') }}" style="color: var(--text-secondary);"
                                            ...
                                            <div class="w-10 h-10 rounded-xl flex items-center justify-center mr-4 transition-colors duration-200"
                                                style="background-color: var(--accent-bg, #dbeafe);">
                                                <i class="fas fa-user text-sm"
                                                    style="color: var(--accent-primary, #2563eb);"></i>
                                            </div>
                                        ...
                                        <a class="flex items-center px-6 py-3 transition-all duration-200 group"
                                            href="{{ route('pengguna-sistem.index') }}"
                                            style="color: var(--text-secondary);"
                                            onmouseover="this.style.backgroundColor='var(--hover-bg)'; this.style.color='var(--accent-secondary, #059669)';"
                                            onmouseout="this.style.backgroundColor='transparent'; this.style.color='var(--text-secondary)';">
                                            <div class="w-10 h-10 rounded-xl flex items-center justify-center mr-4 transition-colors duration-200"
                                                style="background-color: var(--accent-bg-secondary, #d1fae5);">
                                                <i class="fas fa-users text-sm"
                                                    style="color: var(--accent-secondary, #059669);"></i>
                                            </div>
                                            <span class="font-medium">Kelola Users</span>
                                        </a>
                                    </div>
                                    <div class="border-t"
                                        style="border-color: var(--border-primary); background-color: var(--bg-secondary, #f9fafb);">
                                        <form method="POST" action="{{ route('logout') }}" class="hidden" id="logout-form">
                                            @csrf
                                        </form>
                                        <button class="flex items-center w-full px-6 py-3 transition-all duration-200 group"
                                            style="color: var(--danger-primary, #dc2626);"
                                            onmouseover="this.style.backgroundColor='var(--danger-bg, #fef2f2)'; this.style.color='var(--danger-hover, #b91c1c)';"
                                            onmouseout="this.style.backgroundColor='transparent'; this.style.color='var(--danger-primary, #dc2626)';"
                                            onclick="handleLogout(event); return false;">
                                            <div class="w-10 h-10 rounded-xl flex items-center justify-center mr-4 transition-colors duration-200"
                                                style="background-color: var(--danger-bg-light, #fee2e2);">
                                                <i class="fas fa-sign-out-alt text-sm"
                                                    style="color: var(--danger-primary, #dc2626);"></i>
                                            </div>
                ...
                <main class="flex-1 overflow-y-auto" style="background-color: var(--bg-primary);">
                    <div class="py-4">
                        ...
                        @if(session('warning'))
                            <div
                                class="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-lg mb-6 flex items-center justify-between">
                                <div class="flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>
                                    {{ session('warning') }}
                                </div>
                                <button type="button" class="text-yellow-600 hover:text-yellow-800 focus:outline-none"
                                    onclick="this.parentElement.remove()">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                        ...
                        @if(session('info'))
                            <div
                                class="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded-lg mb-6 flex items-center justify-between">
                                <div class="flex items-center">
                    ...
                    <div class="px-4">
                        <div class="flex items-center justify-between text-sm" style="color: var(--text-secondary);">
                            <div class="flex items-center space-x-2">
                                <div
                                    class="w-6 h-6 bg-gradient-to-br from-gray-200 to-gray-200 rounded-lg flex items-center justify-center">
                                    <i class="fas fa-recycle text-emerald-400 text-xs"></i>
                                </div>
                                <span>Copyright &copy; WASPRO {{ date('Y') }}</span>
                            </div>
                            <div class="flex items-center space-x-6">
                                <a href="#" class="hover:text-blue-600 transition-colors duration-200 font-medium">Privacy
                                    Policy</a>
                                <span style="color: var(--border-primary);">&middot;</span>
                                <a href="#" class="hover:text-blue-600 transition-colors duration-200 font-medium">Terms
                                    &amp; Conditions</a>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function saveSectionState(sectionId, isExpanded) {
            const states = JSON.parse(localStorage.getItem('sidebar_section_states') || '{}');
            states[sectionId] = isExpanded;
            localStorage.setItem('sidebar_section_states', JSON.stringify(states));
        }

        // Load section states from localStorage
        function loadSectionStates() {
            const states = JSON.parse(localStorage.getItem('sidebar_section_states') || '{}');
            const sections = ['laporan', 'master-data', 'limbah', 'pengaturan'];
            ...
            sections.forEach(sectionId => {
                ...
                if (section && chevron) {
                    // Check if this section contains active route
                    const hasActiveRoute = section.querySelector('.bg-blue-600\\/80, .bg-purple-600\\/80, .bg-orange-600\\/80, .bg-green-600\\/80, .bg-yellow-600\\/80');
                    const shouldExpand = hasActiveRoute || states[sectionId];
        ...
        // Initialize sidebar accordion on page load
        document.addEventListener('DOMContentLoaded', function() {
            loadSectionStates();
        });
    </script>
