                        ...
                        <table class="min-w-full w-full table-hover-rows">
                            <thead style="background-color: var(--card-secondary-bg);">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                                        style="color: var(--text-secondary);">Efisiensi (%)</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                                        style="color: var(--text-secondary);">Rata-rata Hari Penyimpanan</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                                        style="color: var(--text-secondary);">Tersimpan</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                                        style="color: var(--text-secondary);">Diangkut</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                                        style="color: var(--text-secondary);">Jenis Limbah Terbanyak</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($unitStats as $stats)
                                    <tr class="border-b" style="border-color: var(--border-primary);">
                                        <td class="px-4 py-3 text-sm font-medium text-primary-var">{{ $stats['nama_unit'] }}</td>
                                        ...
                                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                                            {{ number_format($stats['efficiency_rate'], 2) }}</td>
                                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                                            {{ number_format($stats['avg_storage_days'], 1) }}</td>
                                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                                            {{ $stats['status_breakdown']['Tersimpan'] ?? 0 }}</td>
                                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                                            {{ $stats['status_breakdown']['Diangkut'] ?? 0 }}</td>
                                        <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                                            @php $top = $stats['waste_types']->first(); @endphp
                                            {{ $top ? ($top['nama_limbah'] ?? '-') : '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="mt-2 overflow-hidden rounded-2xl border shadow-sm"
                ...
                <table class="min-w-full w-full table-hover-rows">
                    ...
                    <tbody>
                        @forelse($logs as $index => $log)
                            <tr class="border-b" style="border-color: var(--border-primary);">
                                ...
                                <td class="px-4 py-3 text-sm font-medium text-primary-var">
                                    {{ $log->unitPembangkit?->nama_unit ?? '-' }}</td>
                                <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-6 text-center text-sm" style="color: var(--text-secondary);">Tidak
