...
<body>
    <div class="header">
        ...
        <p>Periode: {{ $dateFrom ? date('d/m/Y', strtotime($dateFrom)) : 'Semua' }} - {{ $dateTo ? date('d/m/Y', strtotime($dateTo)) : 'Semua' }}</p>
        <p>Tanggal Cetak: {{ date('d/m/Y H:i:s') }}</p>
    </div>

    <div class="info">
        <p><strong>Filter:</strong></p>
        <p>Jenis Limbah: {{ $jenisLimbahId ? ($logs->first()->jenisLimbah->nama_limbah ?? 'Unknown') : 'Semua Jenis Limbah' }}</p>
        <p>Periode: {{ $dateFrom ? date('d/m/Y', strtotime($dateFrom)) : 'Tidak dibatasi' }} s/d {{ $dateTo ? date('d/m/Y', strtotime($dateTo)) : 'Tidak dibatasi' }}</p>
    </div>
    ...
    @else
    <p style="text-align: center; margin-top: 50px; font-style: italic;">Tidak ada data yang sesuai dengan filter yang dipilih.</p>
    @endif
</body>
</html>