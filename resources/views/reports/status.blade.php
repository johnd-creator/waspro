                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        <div>
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="Tersimpan" {{ request('status') === 'Tersimpan' ? 'selected' : '' }}>Tersimpan
                                </option>
                                <option value="Diangkut" {{ request('status') === 'Diangkut' ? 'selected' : '' }}>Diangkut
                                </option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
                        <div class="rounded-2xl p-4" style="background-color: var(--border-primary);">
                            ...
                            <div class="mt-1 text-2xl font-semibold" style="color: var(--text-primary);">
                                {{ $statusDistribution['Tersimpan']['count'] ?? 0 }}</div>
                            <div class="text-xs" style="color: var(--text-secondary);">
                                {{ number_format($statusDistribution['Tersimpan']['total_quantity'] ?? 0, 2) }} Kg</div>
                        </div>
                        <div class="rounded-2xl p-4" style="background-color: var(--border-primary);">
                            <div class="text-xs font-medium" style="color: var(--text-secondary);">Diangkut</div>
                            <div class="mt-1 text-2xl font-semibold" style="color: var(--text-primary);">
                                {{ $statusDistribution['Diangkut']['count'] ?? 0 }}</div>
                            <div class="text-xs" style="color: var(--text-secondary);">
                                {{ number_format($statusDistribution['Diangkut']['total_quantity'] ?? 0, 2) }} Kg</div>
                        </div>
                        <div class="rounded-2xl p-4" style="background-color: var(--border-primary);">
                            <div class="text-xs font-medium" style="color: var(--text-secondary);">Kadaluarsa</div>
                            <div class="mt-1 text-2xl font-semibold" style="color: var(--text-primary);">
                                {{ $statusDistribution['Kadaluarsa']['count'] ?? 0 }}</div>
                ...
                <table class="min-w-full w-full table-hover-rows">
                    <thead style="background-color: var(--border-primary);">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                                style="color: var(--text-secondary);">Maksimal Penyimpanan</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                                style="color: var(--text-secondary);">Hari Tersisa</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($logs as $index => $log)
                            ...
                            <tr class="border-b {{ $isExpired ? 'expired-row' : '' }}" style="border-color: var(--border-primary);">
                                ...
                                <td class="px-4 py-3 text-sm" style="color: var(--text-secondary);">
                                    {{ $log->maksimal_penyimpanan_tanggal ? \Carbon\Carbon::parse($log->maksimal_penyimpanan_tanggal)->format('d M Y') : '-' }}
                                </td>
