            ...
            <div class="p-6">
                ...
                <form method="GET" action="{{ route('reports.monthly') }}" class="mb-6">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        <div>
                            <label for="year" class="mb-1 block text-sm font-medium"
                                style="color: var(--text-secondary);">Tahun</label>
                        ...
                        <div>
                            <label for="month" class="mb-1 block text-sm font-medium"
                                style="color: var(--text-secondary);">Bulan (Opsional)</label>
                        ...
                        <div>
                            <label for="unit_id" class="mb-1 block text-sm font-medium"
                                style="color: var(--text-secondary);">Unit Pembangkit</label>
                ...
                @if($hasData)
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
                        ...
                        <div class="rounded-2xl p-4" style="background-color: var(--border-primary);">
                            ...
                            <div class="mt-1 text-2xl font-semibold" style="color: var(--text-primary);">{{ $totalTransported }}
                            </div>
                        </div>
                        <div class="rounded-2xl p-4" style="background-color: var(--border-primary);">
                            <div class="text-xs font-medium" style="color: var(--text-secondary);">Tersimpan</div>
                            <div class="mt-1 text-2xl font-semibold" style="color: var(--text-primary);">{{ $wasteStored }}
                            </div>
                        </div>
                        <div class="rounded-2xl p-4" style="background-color: var(--border-primary);">
                            ...
                            <div class="mt-1 text-2xl font-semibold" style="color: var(--text-primary);">
                                {{ number_format($totalWaste, 2) }}</div>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="mt-2 overflow-hidden rounded-2xl border shadow-sm"
                ...
                <table class="min-w-full w-full table-hover-rows">
                    ...
                    <tbody>
                        @forelse($logs as $index => $log)
                            <tr class="border-b" style="border-color: var(--border-primary);">
                                ...
                                <td class="px-4 py-3 text-sm font-medium text-primary-var">
                                    {{ $log->jenisLimbah?->nama_limbah ?? 'Unknown' }}</td>
                                ...
                                <td class="px-4 py-3">
                                    @php 
                                        ...
                                        $isTransported = $statusValue === \App\Enums\LogStatus::Diangkut->value; 
                                    @endphp
                                    <span
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-4 py-6 text-center text-sm" style="color: var(--text-secondary);">Tidak
                                    ada data untuk periode yang dipilih</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
